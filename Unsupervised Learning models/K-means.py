...
class K_means():
    ...
    def __init__(self):
        ...
        self.__y = None
        self.__model = None
        self.__k_means_labels = None
        self.__k_means_cluster_centers = None
        
        
    def create_syntetic_data(self):
        '''Create syntetic data with make_blobs function'''
        self.__X, self.__y = make_blobs(n_samples=5000, centers=[[4,4], [-2, -1], [2, -3], [1, 1]], cluster_std=0.9)
    ...
    def train_model(self,n__clusters = 4, n__init = 12):
        ...
        self.__model = KMeans(init = "k-means++", n_clusters = self.__n_clusters, n_init = self.__n_init)
        self.__model.fit(self.__X)
        
        self.__k_means_labels = self.__model.labels_
        print(self.__k_means_labels)
        
        self.__k_means_cluster_centers = self.__model.cluster_centers_
        print(self.__k_means_cluster_centers)
    ...
    def visual_plot_Kmeans(self):
        try:
            ...
            colors = plt.cm.tab10(np.linspace(0, 1, len(set(self.__k_means_labels))))
            ...
            for k, col in zip(range(len([[4, 4], [-2, -1], [2, -3], [1, 1]])), colors):
                ...
                my_members = (self.__k_means_labels == k)

                #def the centroid or cluster center
                cluster_center = self.__k_means_cluster_centers[k]

                #plots the data p. with color col.
                ax.plot(self.__X[my_members, 0], self.__X[my_members, 1], 'w', markerfacecolor=col, marker='.',ms=10)
            ...
            print(f"Error in visualization plot Kmeans !!!")
